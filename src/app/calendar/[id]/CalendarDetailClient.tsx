    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-picks-bg">
        <div className="text-center">
          ...
          <button onClick={() => router.back()} className="mt-4 text-[13px] font-semibold" style={{ color: '#D6536D' }}>
            돌아가기
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-picks-bg page-fade">
      ...
      >
        ...
        <button onClick={() => editing ? handleSave() : setEditing(true)} className="text-[14px] font-semibold" style={{ color: '#D6536D' }}>
          {editing ? '저장' : '편집'}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-32" style={{ paddingTop: '56px' }}>
        ...
        <div className="px-7 mt-5 space-y-3">
          {editing ? (
            <>
              <div className="picks-card p-4">
                ...
                <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full text-[16px] font-semibold text-picks-dark bg-gray-50 rounded-xl px-3 py-2 border border-gray-200 focus:border-picks-rose transition-colors" />
              </div>
              <div className="picks-card p-4">
                <p className="text-[12px] font-semibold text-gray-400 mb-2 uppercase tracking-wide">날짜</p>
                <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full text-[15px] text-picks-dark bg-gray-50 rounded-xl px-3 py-2 border border-gray-200 focus:border-picks-rose transition-colors" />
              </div>
              <div className="picks-card p-4">
                <p className="text-[12px] font-semibold text-gray-400 mb-2 uppercase tracking-wide">메모</p>
                <textarea value={memo} onChange={(e) => setMemo(e.target.value)} rows={3} className="w-full text-[14px] text-picks-dark bg-gray-50 rounded-xl px-3 py-2 border border-gray-200 focus:border-picks-rose transition-colors resize-none" />
              </div>
              <div className="picks-card p-4">
                <p className="text-[12px] font-semibold text-gray-400 mb-3 uppercase tracking-wide">색상</p>
                <div className="flex gap-3">
                  {colorOptions.map((c) => (
                    <button key={c} onClick={() => setColor(c)} className="w-9 h-9 rounded-full transition-transform active:scale-90 flex items-center justify-center" style={{ background: c, border: color === c ? '3px solid #1a1a1a' : '3px solid transparent' }}>
                      {color === c && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <>
              <div className="picks-card p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="px-3 py-1 rounded-full text-[12px] font-semibold" style={{ background: diff < 0 ? '#f5f5f5' : diff === 0 ? '#fdf0f2' : '#f0f8ff', color: diff < 0 ? '#9e9e9e' : diff === 0 ? '#D6536D' : '#2196F3' }}>
                    {diff < 0 ? `${Math.abs(diff)}일 전` : diff === 0 ? '오늘' : `D-${diff}`}
                  </span>
                </div>
              </div>
              <div className="picks-card p-4">
                <p className="text-[12px] font-semibold text-gray-400 mb-2 uppercase tracking-wide">메모</p>
                <p className="text-[14px] text-picks-dark leading-relaxed">{event.memo || <span className="text-gray-400">메모 없음</span>}</p>
              </div>
              <div className="picks-card p-4">
              ...
              {linked && (
                <div className="picks-card p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style={{ background: linked.avatar_color }}>
          ...
          <button onClick={() => setConfirmDelete(true)} className="w-full py-3.5 rounded-2xl font-semibold text-[15px] border-2 transition-all active:scale-95 mt-2" style={{ borderColor: '#E43D12', color: '#E43D12' }}>
            일정 삭제
          </button>
        </div>
      </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.4)' }}>
          <div className="w-full max-w-[393px] bg-white rounded-t-3xl p-6 bottom-sheet">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(false)} className="flex-1 py-3.5 rounded-2xl font-semibold text-[15px] bg-gray-100 text-gray-600">취소</button>
              <button onClick={handleDelete} className="flex-1 py-3.5 rounded-2xl font-semibold text-[15px] text-white" style={{ background: '#E43D12' }}>삭제</button>
            </div>
          </div>
        </div>
      )}
      ...
      {toast && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 toast-enter">
          <div className="bg-picks-dark text-white px-5 py-3 rounded-2xl shadow-lg text-[14px] font-medium whitespace-nowrap">{toast}</div>
        </div>
      )}
    </div>
  );
